...
<script>
  ...
  function generateReport() {
    // 1) fetch every supply from the server
    fetch("../fetchsupplies.php")
      .then(res => res.json())
      .then(data => {
        ...
        logo.onload = function () {
          ...
          // 3) build rows from the JSON payload
          const rows = data.map(supply => [
            supply.code,
            supply.item_name,
            supply.category || "N/A",
            supply.box ? `${supply.box} (boxes)` : "N/A",
            supply.unit ? `${supply.unit} (strips)` : "N/A",
            supply.quantity,
            supply.expiration || "",
            supply.date_added
          ]);

          // 4) autoTable with full dataset
          doc.autoTable({
            head: [[
              "Code",
              "Brand Name",
      ...
      .catch(err => {
        console.error("Error fetching supplies for report:", err);
...
<!-- scoped CSS to only bump the alertModal -->
<style>
  /* only bump the alertModal above everything else */
  #alertModal.modal {
    z-index: 3000;
  }
...
<div class="modal fade" id="updateExpiryModal" tabindex="-1" aria-labelledby="updateExpiryModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" style="overflow: visible;">
    <form id="updateExpiryForm">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="updateExpiryModalLabel">Update Expiration Dates</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body container-fluid" id="updateExpiryBody" style="overflow: visible;">
          <!-- will be populated on show -->
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
...
<script>
// override window.alert to use Bootstrap modal
...
document.addEventListener("DOMContentLoaded", () => {
  const modalEl = document.getElementById("updateExpiryModal");
  const form    = document.getElementById("updateExpiryForm");
  const body    = document.getElementById("updateExpiryBody");

  // 1) When the modal is shown, load current expiries
  modalEl.addEventListener("shown.bs.modal", () => {
    body.innerHTML = '<p class="text-center">Loading…</p>';
    fetch("../fetchsupplies.php")
      .then(r => r.json())
      .then(data => {
        body.innerHTML = data.map(supply => `
          <div class="row mb-3 align-items-center">
            <label class="col-sm-6 col-form-label">${supply.item_name}</label>
            <div class="col-sm-6">
              <input
                type="date"
                class="form-control"
                name="expiration[${supply.code}]"
                value="${supply.expiration||''}"
              >
            </div>
          </div>
        `).join("");
      ...
      .catch(err => {
        ...
        body.innerHTML = '<div class="alert alert-danger">Failed to load supplies.</div>';
      });
  });

  // 2) On submit, hide this modal then POST and handle JSON/text safely
  form.addEventListener("submit", async e => {
    ...
    try {
      const res = await fetch("../updateexpiries.php", {
        method: "POST",
        body: payload,
      ...
      console.log("updateexpiries.php returned raw:", JSON.stringify(text));

      // try parse JSON if any text
      let json;
      try {
        json = text.trim() ? JSON.parse(text) : { status: 'error', message: 'Empty response from server' };
      } catch (parseErr) {
      ...
      if (json.status === "success") {
        alert("Expiration dates updated.");
      } else {
        alert("Error updating expiries: " + (json.message || "Unknown error"));
      }
    } catch (err) {
      console.error("Error updating expiries:", err);
      alert("Error updating expiries: " + err.message);
    }
  });
});
</script>
